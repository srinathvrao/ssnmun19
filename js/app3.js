	...
	$scope.submitForm = function(isValid) {
		...
		function postToGoogle() {
			...
			$.ajax({
				...
				statusCode: {
					0: function() {
						showThanks();
						alert("Google Sheet Entry Successful!!");
					},
					200: function() {
						showThanks();
						alert("Google Sheet Entry Successful!!");
		...
		function backupMail() {
			$.ajax({
				...
				success: function() {
					alert("Backup Mail Successfully Sent!!");
				},
				error: function(){
					alert("Backup Mail Error");
				}
			});
		}
		...
		function validCountries() {
			...
			for(var i=0;i<com1.length;++i)
				if(com1[i].value == country1)
					break;
			if(i == com1.length) {
				alert("Country Preference 1 is not in "+$('#comPref1').val()+".\nPlease enter a valid country");
				return false;
			}
			...
			for(i=0;i<com2.length;++i)
				if(com2[i].value == country2)
					break;
			if(i == com2.length) {
				alert("Country Preference 2 is not in "+$('#comPref2').val()+"\nPlease enter a valid country");
				return false;
			}
			...
			for(i=0;i<com3.length;++i)
				if(com3[i].value == country3)
					break;
			if(i == com3.length) {
				alert("Country Preference 3 is not in "+$('#comPref3').val()+"\nPlease enter a valid country");
				return false;
			}
